...
$(document).ready(function() {
  ...
  var onoff=true; // true=>타이머 동작중 , false=>동작하지 않을때
     ...
     $('.btn1').css('background','#004b84'); //첫번째 불켜, 이미지 바꾸기 대신 css 수정
     $('.btn1').css('width','30');

     $('.visual_textbox li:eq(0)').fadeIn('slow');    
 ...
 function moveg(){
      cnt+=direct;  //카운트가 1 2 3 4 5 4 3 2 1 2 3 4 5 ......
       ...
       $('.visual_textbox li').hide();
       $('.visual_textbox li:eq('+ (cnt-1) +')').fadeIn('slow');
      ...
      $('.btn'+cnt).css('background','#004b84');//자신만 불켜
  ...
  $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
    ...
    $('.btn'+cnt).css('background','#004b84');//자신 버튼만 불켜 
    $('.btn'+cnt).css('width','30');
